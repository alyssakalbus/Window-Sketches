...
function setup() {
  ...
  /*
    Here we are trying to get access to the camera. We will try two different
    methods. Unfortunately, for example, iOS devices seem particularly himeric
    and when working with them, P5js often can not access the camera.
  */
  var temp_captureReadyFlag = false;
  try {
    myCapture = createCapture(VIDEO);
    myCapture.size(320, 240);
    myCapture.elt.setAttribute('playsinline', '');
    myCapture.hide();
    console.log('[setup] capture [method 1] ready.');
    temp_captureReadyFlag = true;
  } catch(_err) {
    console.log('[setup] capture [method 1] error: ' + _err);
  }
  if(!temp_captureReadyFlag) {
    try {
      myCapture = createCapture(
        {
          audio: false,
          video: {width: 320, height: 240},
          mandatory: {minWidth: 320, minHeight: 240},
          facingMode: "user"
        },
        function() {
          myCapture.elt.setAttribute('playsinline', '');
          myCapture.hide();
          console.log('[setup] capture [method 2] ready.');
          temp_captureReadyFlag = true;
        }
      );
    } catch(_err) {
      console.log('[setup] capture [method 2] error: ' + _err);
    }
  }
 ...
 */
  ...
  */
  myVida.normMinBlobArea = 0.02;  // uncomment if needed
  myVida.normMaxBlobArea = 0.95;  // uncomment if needed
  /*
